...
function update_queue(idx) {
	queue.id = idx
	audio.src = queue.songs[queue.id].url
	audio.addEventListener("ended", () => {
		//if (isPlaying) {
		if (queue.id >= queue.songs.length - 1) {
			update_queue(0)
		} else {
			update_queue(idx + 1)	
		}
	...
	isPlaying = true
	audio.play();
	audioBtn.textContent = `[ ${queue.songs[queue.id].name} ]`
...
export function send_queue(ext_queue) {
	queue = ext_queue
	update_queue(queue.id)
}

export function pause_play() {
	audio.pause();
    isPlaying = false; 
}
